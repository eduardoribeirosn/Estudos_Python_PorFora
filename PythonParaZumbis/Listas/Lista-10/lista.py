...
def near_ten(n):
    ...
    for i in range(n + 3):
        if i % 10 == 0:
            listMult.append(i)
    for item in listMult:
        if n >= (item - 2) and n <= (item + 2):
            return True
    return False
